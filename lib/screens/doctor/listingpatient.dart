...
class _ListpatState extends State<Listpat> {
  ...
  getAllPatient() async{
    ...
    GetPatient().getAllPatients().then((val) {
      if (val.data['success']) {
        List fiches=val.data["fiches"] ;
        print(fiches);
        Future.delayed(Duration(seconds: 2)).then((value) => setState(() {
          loading = false;
          allPatients = val.data["fiches"];
        }));
      }
    }
    );
          ...
          itemBuilder: (BuildContext context, index){
            return Padding(
              ...
              child: Container(
                ...
                child: ListTile(
                  onTap: (){
                    Navigator.push(context, MaterialPageRoute(builder: (context) => PatientDetailsScreen(patientDetails:allPatients[index],)));
                  ...
                  title: Text("${allPatients[index]["firstname"]} ${allPatients[index]["lastname"]}"),
                  subtitle: Text("${allPatients[index]["email"]} "),
